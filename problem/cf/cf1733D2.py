...
代码实现时，为了方便处理 y/2y/2，可以把所有数都乘 22，最后再除以 22。

此外，计算 ff 的过程可以用两个变量滚动计算。

答疑
问：为什么花费 yy 的时候，不用考虑相邻的情况？

答：因为 y>xy>x，如果相邻，从 f[i-2]f[i−2] 转移过来是更优的。

问：转移方程没有对选了多少个 yy 加以限制，如果选了奇数个 yy 怎么办？

答：注意 mm 是偶数，那么一定会选偶数个 yy，你可以从记忆化搜索的角度来思考这一点。
"""
