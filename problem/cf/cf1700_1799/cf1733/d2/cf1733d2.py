...
如果 m=2 且 p[0]+1=p[1]，则答案为 min(2y,x)，因为可以找个不相邻的位置充当「中转站」（注意n≥5）；
否则答案为 m/2⋅y，方案是把 p[i] 和 p[i+m/2] 一组。
提示 4
对于 y>x，可以用动态规划求解。
...
找个不相邻的位置操作，花费 y，那么对于 p[i] 相当于花费 y/2，因此
f[i]=f[i−1]+y/2
不断找相邻的位置操作，把 p[i] 和 p[i−1] 一组，那么需要操作 p[i]−p[i−1] 次，因此
