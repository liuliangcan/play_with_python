# Created by Bob at 2023/03/14 16:53
# https://leetcode.cn/problems/k-th-smallest-prime-fraction/

"""
786. 第 K 个最小的素数分数 (Medium)
...
那么第 `k` 个最小的分数是多少呢?  以长度为 `2` 的整数数组返回你的答案, 这里 `answer[0] ==
arr[i]` 且 `answer[1] == arr[j]` 。
...
class Solution:
    def kthSmallestPrimeFraction(self, arr: List[int], k: int) -> List[int]:
        n = len(arr)
        h = [(1 / arr[i], 0, i) for i in range(n-1,0,-1)]
        k -= 1
        while k:
            _, i, j = heappop(h)
            i += 1
