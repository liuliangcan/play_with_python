# Created by Bob at 2023/01/31 17:38
...
当前勇者需要完成「采集若干批果实」的任务， \`fruits\[j\] = \[type, num\]\` 表示第
\`j\` 批需要采集 \`num\` 颗 \`type\` 类型的果实。采集规则如下：
\- 按 \`fruits\` 给定的顺序\*\*依次\*\*采集每一批次
\- 采集完当前批次的果实才能开始采集下一批次
\- 勇者完成当前批次的采集后将\*\*清空背包\*\*（即多余的果实将清空）
请计算并返回勇者完成采集任务最少需要的时间。
\*\*示例 1：\*\*
>输入：\`time = \[2,3,2\], fruits =
\[\[0,2\],\[1,4\],\[2,1\]\], limit = 3\`
>
>输出：\`10\`
>
...
>返回总耗时 2+6+2=10
\*\*示例 2：\*\*
>输入：\`time = \[1\], fruits = \[\[0,3\],\[0,5\]\], limit =
2\`
>
>输出：\`5\`
>
