...
def solve1(n, a):
    h = sorted(set(a))
    tree = BinIndexTree(len(h))
    ans = 0
    for i, v in enumerate(a):
        x = bisect_left(h, v) + 1
        if i:
            ans = (ans + quick_pow_mod(2, i - 1) * tree.sum_prefix(x)) % MOD
        tree.add_point(x, quick_pow_mod(quick_pow_mod(2, i), MOD - 2, MOD))
...
def solve2(n, a):
    ...
    inv_2j = [1] * n  # 预处理2**i的逆元，显然i==0时，2**i=1 1的逆元是1
    inv_2j[n - 1] = quick_pow_mod(power[n - 1], MOD - 2, MOD)  # 计算2**(n-1)的逆元
    for i in range(n - 2, 0, -1):  # 逆序计算逆元，2**j = 2**(j+1) // 2,两边取倒数 即inv[j] = inv[j+1]*2
        inv_2j[i] = inv_2j[i + 1] * 2 % MOD
    ...
    for i, v in enumerate(a):
        x = bisect_left(h, v) + 1
        if i:
            ans = (ans + power[i - 1] * tree.sum_prefix(x)) % MOD
        tree.add_point(x, inv_2j[i])
...
def solve3(n, a):
    ...
    for i, v in enumerate(a):
        x = bisect_left(h, v) + 1
        if i:
            ans = (ans + pwr2 * tree.sum_prefix(x)) % MOD
            pwr2 = pwr2 * 2 % MOD
        tree.add_point(x, inv)
        inv = inv * inv2 % MOD
