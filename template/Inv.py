...
1~n的逆元线性递推：
百度之星20240602 B:n个人，每个人跑1圈的要花i分钟，问所有人两两相遇几次。答案取模
显然最后时间是lcm分钟。设一圈长度是1，那么他们的速度分别是1/1,1/2,1/3。
对于x<y的两个人来说，他们的速度差是1/x-1/y,在lcm分钟，一共追记(1/x-1/y)圈，即相遇次数。
那么 对于一个i来说，比它大的数有n-i个，即贡献n-i次=(n-i)/i,
        比它小的有i-1个，即贡献(i+1)/i
        最后统一乘LCM即可
只需要解决两个问题：
1. 求1~n每个数的逆元：inv[i] = (MOD - MOD // i) * inv[MOD % i] % MOD
2. 求1~n的lcm:求出<=n的所有质数，然后把每个质数的k次方加到lcm里即可。
