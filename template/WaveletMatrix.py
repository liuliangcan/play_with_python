"""小波矩阵
查询区间第K小/区间内x出现的频次/区间内某个值域范围内的权值和

原理是把原数据按照每一位01稳定排序，并且把每层记录下来。然后按照01的数量不断去掉一部分，去逼近答案。
py线段树递归卡常，但这个可以。
本模板：
    - 注意是0-indexed,不管是[l,r),还是[low,upper)都是左闭右开。
        - wm = WaveletMatrix(self, A: List[int], weight: List[int] = None) 用原数组A初始化小波矩阵，(weight可以为空，但求和就不能用,所以如果想求和，把weight变成原数组)
        - access(self, i: int)  回第i个值
        - rank(self, l: int, r: int, x: int) [l, r)区间中x的个数
        - quantile(self, l: int, r: int, n: int) 返回[l,r)里区间第n小。return n-th (0-indexed) smallest value in [l, r) range
        - rquantile(self, l: int, r: int, n: int) 返回[l,r)里区间第n大  return n-th (0-indeed) largest value in [l, r) range
        - range_freq(self, l: int, r: int, lower: int, upper: int) 返回[l,r)里值在[lower,upper)之间的数字个数
        - prev_value(self, l: int, r: int, upper: int) 返回[l,r)之间小于upper的最大值
        - next_value(self, l: int, r: int, lower: int) 返回[l,r)之间大于等于lower的最小值
        - range_sum(self, l: int, r: int, lower: int, upper: int)  返回[l,r)之间，值在[lower,upper)之间的数的权值。注意必须传入weight才能用
        - range_least_sum(self, l: int, r: int, n: int)  返回[l,r)里，最小的n个数的权值和，**注意这里n是1-indexed**
其他：
    - 矩阵的行数和U有关，因此离散化后可以使行数控制在20以内(一般题目n<1e6)。不离散的话1e9是30，因此可以加速33%。本模板自带这个优化了不用额外做。
讲解：
    - https://zhuanlan.zhihu.com/p/590974585
模板：
    - https://www.luogu.com.cn/record/139671180
    - https://www.luogu.com.cn/record/146228996
例题：
...
class BitVector:
    ...
    TABLE = bytes(
        [0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4,
         ...
         3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 3,
         4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4,
         4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5,
         4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7, 2, 3, 3, 4, 3, 4, 4, 5, 3,
         4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5,
...
class WaveletMatrix:
    ...
    def rquantile(self, l: int, r: int, n: int):
        """return n-th (0-indexed) largest value in [l, r) range"""
