...
3. 本质不同的子串数（s中有多少不同的子串）O(n^2)
    - 这题可以字符串哈希后每个长度滑窗枚举,也是O(n^2)
    - 用Z函数DP，当前已知s的答案时，考虑在后边增加一个新字符c，形成串t(显然这个长度是不同的)，
        现在只需讨论，有多少以c为结尾的串是没出现过（或出现过的）。
        那么从c开始向前扩展，看看每个串前边是否出现过即可，最终直到没出现，后边的就都不会出现了。
        计算t'(t[::-1])的Z函数。ans+=len(t)-max(z)即可。
简介(和manacher很像)：
