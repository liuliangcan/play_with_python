"""
这是个SortedList的简短版实现
底层类似B+树，但是只有一层平铺的List[List[]]。内部小数组负载因子默认200，小数组长度超过400时会分裂为一个200的和一个201的数组。
和官方实现不同的点主要有：
1. 负载因子200而不是1000.官方的1000进行del操作时常数略大。
2. 这里维护的是mins官方维护的是maxs。
3. 这里用一个权值树状数组维护每个小数组的长度，这样找第k小时，可以O(lgm)快速定位在哪个数组里;而官方的实现更类似一个二叉堆
4. 数组数量发生变化时，树状数组需要重新建树，这里建树是O(m)操作 m是数组数量。
4. 不支持del s[l:r]的批量删除操作;不支持切片取
---
抽空打算改造一下让它支持切片取和删除。
"""
